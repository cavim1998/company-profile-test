...
export const useBlog = create<BlogTypes>((set, get) => ({
  ...
  getBlogs: () => {
    ...
    const url = !search
      ? `https://deluxeyoke-us.backendless.app/api/data/Blogs?${defaultParams}`
      : `https://deluxeyoke-us.backendless.app/api/data/Blogs?${defaultParams}&where=${encodeURIComponent(
          searchUrl
        )}`;
    axios
      .get(url)
  ...
  getBlogsDetail: async (id) => {
    ...
    axios
      .get(`https://deluxeyoke-us.backendless.app/api/data/Blogs/${id}`)
      .then((response) => {
  ...
  postArticle: async (data) => {
    try {
      ...
      const response: AxiosResponse<BackendlessError | BackendlessObject> =
        await axios.post(
          "https://deluxeyoke-us.backendless.app/api/data/Blogs",
          {
            ...data,
          }
        );
